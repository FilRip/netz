...
namespace netz
{
    public class InputParser
    {
        ...
        public static ArrayList Parse(ref GenData genData, string[] args)
        {
                ...
                if ((args[i][0] == '-') || (args[i][0] == '/'))
                {
                    ...
                    switch (arg)
                    {
                        ...
                        case "srp":
                            ...
                            break;
                        case "e":
                            genData.Extension = args[++i];
                            break;
                        case "param":
                            genData.ParametresExtension = args[++i];
                            break;
                        case "obfuscator":
                            genData.Obfuscateur = true;
                            genData.NiveauObfuscation = int.Parse(args[++i]);
                            break;
                        case "unsafe":
                            genData.AutoriseCodeUnsafe = true;
                            break;
                        case "manifest":
                            genData.ManifestResourceFile = args[++i];
                            break;
                        default:
            ...
            try
            {
                foreach (InputFile fichier in files)
                {
                    if (File.Exists(fichier.File))
                    {
                        CorFlagsReader retour;
                        retour = CorFlagsReader.ReadAssemblyMetadata(fichier.File);
                        if ((retour != null) && (retour.GetGeneralMachineType() == GeneralMachineType.X86))
                        {
                            Logger.Log("Assembly x86 détectée, force compile en x86");
                            genData.XPlatform = "x86";
                            break;
                        }
                    }
        ...
        public static void PrintInternalVersion(string zipProvider)
        {
            ...
            Logger.Log("| Compiled with .NET     : 4.5.2");
            try
            {
        ...
        internal static string GetInnerTemplatesFingerPrint(string zipProvider)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (temp != null)
                    {
                        hash = sha.ComputeHash(Encoding.ASCII.GetBytes(temp));
        ...
        private static void SetExeOutPath(string file, ref GenData genData)
        {
            ...
            if (genData.Auto)
            {
                try
                {
                    CorFlagsReader reader = CorFlagsReader.ReadAssemblyMetadata(file);
                    AssemblyConfigurationDetector.SubSystem ss = reader.SubSystem;
                    if (ss == AssemblyConfigurationDetector.SubSystem.WINDOWS_CONSOLE)
                    {
                        genData.Console = true;
                    }
                    else if (ss == AssemblyConfigurationDetector.SubSystem.WINDOWS_GUI)
                    {
                        genData.Console = false;
                    }
                ...
                {
                    Netz.PrintWarning("1004 Cannot determine EXE's subsystem (default is GUI windows EXE) The packed application will fail!: ", ex);
                }
            }
