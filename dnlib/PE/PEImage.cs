...
namespace dnlib.PE
{
    ...
    public sealed class PEImage : IInternalPEImage
    {
            ...
            : this(DataReaderFactoryFactory.Create(filename, mapAsImage), mapAsImage ? ImageLayout.Memory : ImageLayout.File, verify)
        {
            try
            {
                if (mapAsImage && dataReaderFactory is MemoryMappedDataReaderFactory factory)
                    factory.SetLength(peInfo.GetImageSize());
            }
            catch
            {
                Dispose();
        ...
        bool IInternalPEImage.IsMemoryMappedIO
        {
            get
            {
                return dataReaderFactory is MemoryMappedDataReaderFactory creator && creator.IsMemoryMappedIO;
            }
        }
        ...
        ImageDebugDirectory[] ReadImageDebugDirectories()
        {
            try
            {
                ...
                for (int i = 0; i < res.Length; i++)
                    res[i] = new ImageDebugDirectory(ref reader);
                return res;
            }
            catch (IOException)
            {
