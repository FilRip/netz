    ...
    public abstract class ModuleDef : IHasCustomAttribute, IHasCustomDebugInformation, IResolutionScope, IDisposable, IListListener<TypeDef>, IModule, ITypeDefFinder, IDnlibDef, ITokenResolver, ISignatureReaderHelper
    {
        ...
        public TypeDef Find(ITypeDefOrRef typeRef)
        {
            ...
            if (!(typeRef is TypeSpec ts))
                return null;
            if (!(ts.TypeSig is TypeDefOrRefSig sig))
                return null;
        ...
        public IEnumerable<AssemblyRef> GetAssemblyRefs()
        {
            for (uint rid = 1; ; rid++)
            {
                if (!(ResolveToken(new MDToken(Table.AssemblyRef, rid).Raw) is AssemblyRef asmRef))
                    break;
                yield return asmRef;
            }
        }
        ...
        public IEnumerable<ModuleRef> GetModuleRefs()
        {
            for (uint rid = 1; ; rid++)
            {
                if (!(ResolveToken(new MDToken(Table.ModuleRef, rid).Raw) is ModuleRef modRef))
                    break;
                yield return modRef;
            }
        }
        ...
        public IEnumerable<MemberRef> GetMemberRefs(GenericParamContext gpContext)
        {
            for (uint rid = 1; ; rid++)
            {
                if (!(ResolveToken(new MDToken(Table.MemberRef, rid).Raw, gpContext) is MemberRef mr))
                    break;
                yield return mr;
            }
        }
        ...
        public IEnumerable<TypeRef> GetTypeRefs()
        {
            for (uint rid = 1; ; rid++)
            {
                if (!(ResolveToken(new MDToken(Table.TypeRef, rid).Raw) is TypeRef mr))
                    break;
                yield return mr;
            }
        }
        ...
        protected static bool IsGreaterAssemblyRefVersion(AssemblyRef found, AssemblyRef newOne)
        {
            ...
            return foundVer is null || (newVer != null && newVer >= foundVer);
        }
