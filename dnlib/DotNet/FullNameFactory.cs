...
namespace dnlib.DotNet
{
    ...
    public struct FullNameFactory
    {
        ...
        /// </summary>
        /// <returns>The scope type or <c>null</c> if none found</returns>
        public static ITypeDefOrRef ScopeType() =>
            new FullNameFactory().GetScopeType();
        ...
        IType GetDefinitionType(IType type)
        {
            ...
            if (type is TypeSig sig)
            {
                ...
                else if (sig is GenericInstSig gis)
        ...
        {
            if (typeDefOrRef is TypeRef typeref)
                CreateNamespace(typeref, onlyNamespace);
            else if (typeDefOrRef is TypeDef typedef)
                CreateNamespace(typedef, onlyNamespace);
            else if (typeDefOrRef is TypeSpec typespec)
                CreateNamespace(typespec, onlyNamespace);
            else
                sb.Append(NULLVALUE);
        }
...
#pragma warning restore IDE0051
        {
            if (typeDefOrRef is TypeRef typeRef)
                CreateAssemblyQualifiedName(typeRef);
            else if (typeDefOrRef is TypeDef typeDef)
                CreateAssemblyQualifiedName(typeDef);
            else if (typeDefOrRef is TypeSpec typeSpec)
                CreateAssemblyQualifiedName(typeSpec);
            else
                sb.Append(NULLVALUE);
        }
        ...
        IAssembly GetDefinitionAssembly(TypeRef typeRef)
        {
            ...
            else if (scope is TypeRef typeref)
                result = GetDefinitionAssembly(typeref);
            else if (scope is AssemblyRef assemblyref)
                result = assemblyref;
            ...
            else if (scope is ModuleDef moduledef)
                result = moduledef.Assembly;
            else
        ...
        ITypeDefOrRef GetScopeType() => null;
