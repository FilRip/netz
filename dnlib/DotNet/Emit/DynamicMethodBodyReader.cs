    ...
    public class DynamicMethodBodyReader : MethodBodyReaderBase, ISignatureReaderHelper
    {
        ...
        class ReflectionFieldInfo
        {
            ...
            void InitializeField(Type type)
            {
                ...
                if (fieldInfo is null && fieldName2 != null)
            ...
            : base(module.Context)
        {
            ...
            if (obj is DynamicMethod method1)
            {
            ...
            if (!(rslvCodeFieldInfo.Read(obj) is byte[] code))
                throw new Exception("No code");
            codeSize = code.Length;
            if (!(rslvMethodFieldInfo.Read(obj) is SR.MethodBase delMethod))
            ...
            if (!(scopeTokensFieldInfo.Read(scope) is System.Collections.IList tokensList))
        ...
        void UpdateLocals(byte[] localsSig)
        {
            ...
            if (!(SignatureReader.ReadSig(this, module.CorLibTypes, localsSig, gpContext) is LocalSig sig))
                return;
            ...
            if (ehHeader != null)
            {
                ...
                { // DynamicResolver only checks bit 6
                  // Calculate num ehs exactly the same way that DynamicResolver does
                    ...
                    for (int i = 0; i < numHandlers; i++)
                    {
                        ...
                        {
                            HandlerType = (ExceptionHandlerType)reader.ReadUInt16()
                        };
                ...
                else
                {
                    ...
                    for (int i = 0; i < numHandlers; i++)
                    {
                        ...
                        var eh = new ExceptionHandler()
                        {
                            HandlerType = (ExceptionHandlerType)reader.ReadUInt32()
                        };
