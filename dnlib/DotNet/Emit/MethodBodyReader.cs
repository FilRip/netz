...
namespace dnlib.DotNet.Emit
{
    ...
    public sealed class MethodBodyReader : MethodBodyReaderBase
    {
        ...
        {
            if (!(opResolver.ResolveToken(localVarSigTok, gpContext) is StandAloneSig standAloneSig))
                return null;
        ...
        {
            if (!(opResolver.ResolveToken(reader.ReadUInt32(), gpContext) is StandAloneSig standAloneSig))
                return null;
