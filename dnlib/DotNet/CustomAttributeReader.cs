    ...
    public struct CustomAttributeReader
    {
        ...
        object ReadValue(SerializationType etype, TypeSig argType, out TypeSig realArgType)
        {
            ...
            switch (etype)
            {
                ...
                case SerializationType.TaggedObject:
                    ...
                    if (realArgType is SZArraySig arraySig)
                        result = ReadArrayArgument(arraySig);
                    else
                        result = ReadValue((SerializationType)realArgType.ElementType, realArgType, out var _);
                    break;

                // It's ET.Class if it's eg. a ctor System.Type arg type
                case (SerializationType)ElementType.Class:
                    if (argType is TypeDefOrRefSig tdr && tdr.DefinitionAssembly.IsCorLib() && tdr.Namespace == "System")
                    {
        ...
        object ReadEnumValue(TypeSig underlyingType)
        {
            if (underlyingType != null)
            {
                ...
                return ReadValue((SerializationType)underlyingType.ElementType, underlyingType, out var _);
            }
        ...
        CANamedArgument ReadNamedArgument()
        {
            ...
            {
                case SerializationType.Property:
                    isField = false;
                    break;
                case SerializationType.Field:
                    isField = true;
                    break;
                default:
