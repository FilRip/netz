...
namespace dnlib.DotNet
{
    readonly struct GenericArgumentsStack
    {
        ...
        public TypeSig Resolve(uint number)
        {
            ...
            for (int i = argsStack.Count - 1; i >= 0; i--)
            {
                ...
                if (!(typeSig is GenericSig gvar) || gvar.IsTypeVar != isTypeVar)
                    return typeSig;
                result = gvar;
                number = gvar.Number;
            }
    ...
    {
        readonly GenericArgumentsStack typeArgsStack = new GenericArgumentsStack(true);
        readonly GenericArgumentsStack methodArgsStack = new GenericArgumentsStack(false);
