...
namespace Confuser.Renamer.BAML
{
    internal class BAMLAnalyzer
    {
        ...
        {
            if (!properties.TryGetValue(name, out List<PropertyDef> ret))
        ...
        {
            if (!events.TryGetValue(name, out List<EventDef> ret))
        ...
        {
            if (!methods.TryGetValue(name, out List<MethodDef> ret))
        ...
        void PopulateReferences(BamlDocument document)
        {
            ...
            {
                if (typeRefs.TryGetValue(rec.OwnerTypeId, out TypeSig declType))
                {
            ...
            {
                if (clrNs.TryGetValue(rec.XmlNamespace, out List<Tuple<AssemblyDef, string>> clrMap))
                {
        ...
        void AddTypeSigReference(TypeSig typeSig, INameReference<IDnlibDef> reference)
        {
            foreach (ITypeDefOrRef type in typeSig.FindTypeRefs())
            {
                ...
                if (context.Modules.Contains((ModuleDefMD)typeDef.Module))
                {
                    ...
                    if (type is TypeRef ref2)
                        service.AddReference(typeDef, new TypeRefReference(ref2, typeDef));
        ...
        void ProcessBAMLElement(BamlElement root, BamlElement elem)
        {
            ...
            AnalyzeElement?.Invoke(this, elem);
        }
        ...
        TypeDef GetAttributeType(IDnlibDef attr)
        {
            ...
            if (attr is PropertyDef def)
                retType = def.PropertySig.RetType.ToBasicTypeDefOrRef();
            else if (attr is EventDef def1)
                retType = def1.EventType;
            return retType?.ResolveTypeDefThrow();
        ...
        void ProcessElementBody(BamlElement root, BamlElement elem)
        {
            foreach (BamlRecord rec in elem.Body)
            {
                ...
                TypeDef type = null;
                IDnlibDef attr = null;
                if (rec is PropertyRecord propRec)
                {
                    ...
                    if (rec is PropertyWithConverterRecord record)
                    {
                        ProcessConverter(record, type);
                    }
                }
                else if (rec is PropertyComplexStartRecord pcsr)
                {
                    var attrInfo = ResolveAttribute(pcsr.AttributeId);
                    type = attrInfo.Item3;
                    attr = attrInfo.Item1;
                    if (attr != null)
                        type = GetAttributeType(attr);
                }
                else if (rec is ContentPropertyRecord cpr)
                {
                    var attrInfo = ResolveAttribute(cpr.AttributeId);
                    type = attrInfo.Item3;
                    attr = attrInfo.Item1;
                ...
                else if (rec is PropertyCustomRecord customRec)
                {
                ...
                else if (rec is PropertyWithExtensionRecord extRec)
                {
                    ...
                    if (extRec.Flags == 602)
                    {
                        ...
                        if ((short)extRec.ValueId >= 0)
                        {
                            ...
                            {
                                TypeDef declTypeDef;
                                if (typeRefs.TryGetValue(attrInfo.Item2.OwnerTypeId, out TypeSig declType))
                ...
                else if (rec is TextRecord txt)
                {
                    string value = txt.Value;
                    if (txt is TextWithIdRecord record)
                        value = strings[record.ValueId].Value;

                    TypeSig sig = ResolveType(value.Trim(), out _);
        ...
        void ProcessConverter(PropertyWithConverterRecord rec, TypeDef type)
        {
            ...
            else if (converter.FullName == "System.Windows.Input.CommandConverter")
            {
                ...
                if (index != -1)
                {
                    ...
                    TypeSig sig = ResolveType(typeName, out _);
                    if (sig != null)
                    {
            ...
            else if (attrName == "Source")
            {
                ...
                if (attrInfo.Item1 is IMemberDef def)
                    declType = def.DeclaringType.FullName;
                else if (attrInfo.Item2 != null)
        ...
        Tuple<IDnlibDef, AttributeInfoRecord, TypeDef> AnalyzeAttributeReference(TypeDef declType, AttributeInfoRecord rec)
        {
            ...
            return Tuple.Create(retDef, rec, retType?.ResolveTypeDefThrow());
        }
        ...
        void AnalyzePropertyPath(string path)
        {
            ...
            foreach (PropertyPathPart part in propertyPath.Parts)
            {
                ...
                {
                    part.ExtractAttachedDP(out string type, out string property);
                    if (type != null)
                    {
                        TypeSig sig = ResolveType(type, out _);
                        if (sig != null && context.Modules.Contains((ModuleDefMD)sig.ToBasicTypeDefOrRef().ResolveTypeDefThrow().Module))
                        {
                ...
                {
                    if (properties.TryGetValue(part.Name, out List<PropertyDef> candidates))
                ...
                if (part.IndexerArguments != null)
                {
                    foreach (PropertyPathIndexer indexer in part.IndexerArguments)
                        ...
                        {
                            TypeSig sig = ResolveType(indexer.Type, out _);
                            if (sig != null && context.Modules.Contains((ModuleDefMD)sig.ToBasicTypeDefOrRef().ResolveTypeDefThrow().Module))
                            {
                                var reference = new BAMLPathTypeReference(xmlnsCtx, sig, part);
                                AddTypeSigReference(sig, reference);
        ...
        internal class XmlNsContext
        {
            ...
            {
                if (!xmlNsMap.TryGetValue(Tuple.Create(assembly, clrNs), out string prefix))
                {
                    prefix = "_" + x++;
