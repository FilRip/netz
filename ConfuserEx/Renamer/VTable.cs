...
namespace Confuser.Renamer
{
    public class VTableSignature
    {
        ...
        public static VTableSignature FromMethod(IMethod method)
        {
            ...
            if (declType is GenericInstSig sig1)
            {
                sig = GenericArgumentResolver.Resolve(sig, sig1.GenericArguments);
            }
    ...
    public class VTable
    {
        ...
        static void Implements(VTableConstruction vTbl, Dictionary<VTableSignature, MethodDef> virtualMethods, VTable ifaceVTbl, TypeSig iface)
        {
            ...
            VTableSlot implLookup(VTableSlot slot)
            {
    ...
    {
        readonly Dictionary<TypeDef, VTable> storage = new Dictionary<TypeDef, VTable>();
        ...
        public VTable GetVTable(ITypeDefOrRef type)
        {
            if (type == null)
                return null;
            if (type is TypeDef typeDef1)
                return GetOrConstruct(typeDef1);
            ...
            if (type is TypeSpec typeSpec)
            {
                ...
                if (sig is TypeDefOrRefSig sig1)
                {
                    TypeDef typeDef = sig1.TypeDefOrRef.ResolveTypeDefThrow();
                    return GetOrConstruct(typeDef);
                }
                if (sig is GenericInstSig genInst)
                {
