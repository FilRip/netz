    ...
    internal class AnalyzePhase : ProtectionPhase
    {
        ...
        protected internal override void Execute(ConfuserContext context, ProtectionParameters parameters)
        {
            ...
            foreach (IDnlibDef def in parameters.Targets.WithProgress(context.Logger))
            {
                ...
                if (def is TypeDef def2)
                {
                    service.GetVTables().GetVTable(def2);
                    service.SetOriginalNamespace(def, def2.Namespace);
                }
        ...
        {
            if (def is TypeDef def1)
                Analyze(service, context, parameters, def1);
            else if (def is MethodDef def2)
                Analyze(service, context, parameters, def2);
            else if (def is FieldDef def3)
                Analyze(service, context, parameters, def3);
            else if (def is PropertyDef def4)
                Analyze(service, context, parameters, def4);
            else if (def is EventDef def5)
                Analyze(service, context, parameters, def5);
