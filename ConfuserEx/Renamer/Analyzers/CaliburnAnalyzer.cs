...
namespace Confuser.Renamer.Analyzers
{
    internal class CaliburnAnalyzer : IRenamer
    {
        ...
        {
            if (!(def is TypeDef type) || type.DeclaringType != null)
                return;
        ...
        void AnalyzeBAMLElement(BAMLAnalyzer analyzer, BamlElement elem)
        {
            ...
            {
                if (!(rec is PropertyWithConverterRecord prop))
                    continue;
        ...
        {
            if (!(attr.Item1 is PropertyDef def) || def.DeclaringType.FullName != "System.Windows.FrameworkElement")
                return;
