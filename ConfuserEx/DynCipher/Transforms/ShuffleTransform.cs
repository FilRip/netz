...
namespace Confuser.DynCipher.Transforms
{
    internal class ShuffleTransform
    {
        ...
        {
            if (exp is VariableExpression expression)
                yield return expression.Variable;
            else if (exp is ArrayIndexExpression expression2)
            {
                foreach (Variable i in GetVariableUsage(expression2.Array))
                    yield return i;
            }
            else if (exp is BinOpExpression expression3)
            {
                foreach (Variable i in GetVariableUsage(expression3.Left)
                    .Concat(GetVariableUsage(expression3.Right)))
                    yield return i;
            }
            else if (exp is UnaryOpExpression expression4)
            {
                foreach (Variable i in GetVariableUsage(expression4.Value))
                    yield return i;
            }
        }
        ...
        static IEnumerable<Variable> GetVariableUsage(Statement st)
        {
            ...
            {
                foreach (Variable i in GetVariableUsage(statement.Value))
                    yield return i;
            }
        }
        ...
        static IEnumerable<Variable> GetVariableDefinition(Statement st)
        {
            if (st is AssignmentStatement statement)
            {
                foreach (Variable i in GetVariableDefinition(statement.Target))
                    yield return i;
            }
        }
