    ...
    static partial class AntiDebugAntinet
    {
        ...
        private class ProfilerDetectorCLR40 : ProfilerDetector
        {
            ...
            private static unsafe void FixTimeOutOption(IntPtr timeOutOptionAddr)
            {
                ...
                VirtualProtect(timeOutOptionAddr, (int)ConfigDWORDInfo_defValue + 4, PAGE_EXECUTE_READWRITE, out uint oldProtect);
                try
                {
            ...
            private static unsafe IntPtr FindTimeOutOptionAddress()
            {
                try
                {
                    ...
                    if (!peInfo.FindSection(".rdata", out IntPtr sectionAddr, out uint sectionSize) &&
            ...
            private unsafe bool PatchAttacherThreadProc()
            {
                ...
                VirtualProtect(new IntPtr(p), 5, PAGE_EXECUTE_READWRITE, out uint oldProtect);
                try
                {
                    if (IntPtr.Size == 4)
                    {
