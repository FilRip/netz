...
namespace Confuser.Protections.Constants
{
    internal class EncodePhase : ProtectionPhase
    {
        ...
        protected internal override void Execute(ConfuserContext context, ProtectionParameters parameters)
        {
            ...
            {
                if (entry.Key is string chaine)
                {
                    EncodeString(moduleCtx, chaine, entry.Value);
                }
                else if (entry.Key is int entier)
                {
                    EncodeConstant32(moduleCtx, (uint)entier, context.CurrentModule.CorLibTypes.Int32, entry.Value);
                }
                else if (entry.Key is long entierLong)
                {
                    EncodeConstant64(moduleCtx, (uint)(entierLong >> 32), (uint)entierLong, context.CurrentModule.CorLibTypes.Int64, entry.Value);
                }
                else if (entry.Key is float virguleFlottante)
                {
                ...
                else if (entry.Key is double virguleFlottanteLong)
                {
                    var t = new RTransform()
                    {
                        R8 = virguleFlottanteLong
                    };
            ...
            foreach (var type in context.CurrentModule.GetTypes())
                foreach (var method in type.Methods.Where(m => m.HasBody))
                {
                    foreach (var instr in method.Body.Instructions)
                        if (instr.Operand is FieldDef def && !fieldRefs.Contains(instr))
                            dataFields.Remove(def);
                }
