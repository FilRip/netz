...
namespace Confuser.Protections.ReferenceProxy
{
    internal class ExpressionEncoding : IRPEncoding
    {
        ...
        void Compile(RPContext ctx, out Func<int, int> expCompiled, out Expression inverse)
        {
        ...
        {
            if (!keys.TryGetValue(init, out Tuple<Expression, Func<int, int>> ret))
            {
                Compile(ctx, out Func<int, int> keyFunc, out Expression inverse);
