        ...
        public int[] TraceArguments(Instruction instr)
        {
            ...
            while (working2.Count > 0)
            {
                ...
                while (index != instrIndex && index < method.Body.Instructions.Count)
                {
                    ...
                    _ = currentInstr.Operand;
                    if (currentInstr.Operand is Instruction instruction)
                    {
                        int targetIndex = offset2index[instruction.Offset];
                    ...
                    else if (currentInstr.Operand is Instruction[] v)
                    {
                        foreach (Instruction targetInstr in v)
