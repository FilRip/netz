...
namespace Confuser.Core
{
    ...
    public static class DnlibUtils
    {
        ...
        public static ITypeDefOrRef ToBasicTypeDefOrRef(this TypeSig typeSig)
        {
            ...
            if (typeSig is GenericInstSig sig1)
                return sig1.GenericType.TypeDefOrRef;
            if (typeSig is TypeDefOrRefSig sig2)
                return sig2.TypeDefOrRef;
            return null;
        }
        ...
        static void FindTypeRefsInternal(TypeSig typeSig, IList<ITypeDefOrRef> ret)
        {
            while (typeSig.Next != null)
            {
                if (typeSig is ModifierSig sig)
                    ret.Add(sig.Modifier);
                typeSig = typeSig.Next;
            }

            if (typeSig is GenericInstSig genInst)
            {
            ...
            else if (typeSig is TypeDefOrRefSig sig)
            {
                var type = sig.TypeDefOrRef;
                while (type != null)
                {
                    ret.Add(type);
        ...
        public static bool IsArrayAccessors(this IMethod method)
        {
            ...
            if (declType is GenericInstSig sig)
                declType = sig.GenericType;
    ...
    public class ImageStream : Stream
    {
        ...
        public ImageStream(dnlib.IO.DataReader baseStream)
        {
        ...
        public dnlib.IO.DataReader BaseStream;
        ...
        public override long Position
        {
            ...
            set { BaseStream.Position = (uint)value; }
        }
        ...
        {
            BaseStream.ReadBytes(buffer, offset, count);
