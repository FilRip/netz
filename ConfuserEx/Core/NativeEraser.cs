    ...
    internal class NativeEraser
    {
        ...
        static void Erase(List<Tuple<uint, uint, byte[]>> sections, IFileSection s)
        {
            foreach (var sect in sections)
                ...
                {
                    Erase(sect, (uint)s.StartOffset, s.EndOffset - s.StartOffset);
                    break;
                }
        }
        ...
        public static void Erase(NativeModuleWriter writer, ModuleDefMD module)
        {
            ...
            foreach (var origSect in writer.OrigSections)
            {
                ...
                byte[] buf = s.ToArray();
                var newChunk = new DataReaderChunk(new DataReader(DataStreamFactory.Create(buf), 0, (uint)buf.Length), oldChunk.GetVirtualSize());
            ...
            {
                md.TablesStream.TryReadMethodRow(i, out dnlib.DotNet.MD.RawMethodRow method);
